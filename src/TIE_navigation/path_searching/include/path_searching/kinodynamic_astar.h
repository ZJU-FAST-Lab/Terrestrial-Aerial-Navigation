...
class PathNode {
 public:
  ...
  double g_score, f_score, penalty_g_score, proximity_penalty_score;
...
class KinodynamicAstar {
 private:
  ...
  std::vector<PathNodePtr> last_path_nodes_;
  /* ---------- record data ---------- */
  ...
  double ground_occupied_cost_, proximity_cost_, flying_cost_, flying_cost_base_;
  /* ---------- parameter ---------- */
  /* search */
 ...
 public:
  ...
  double solveSwitchPoint();
  void getHybridSamples(double& ts, vector<vector<Eigen::Vector3d>>& hybrid_point_set_list,
                                  vector<Eigen::Vector3d>& start_end_derivatives, 
                                   vector<int>& hybrid_motion_list);
  std::vector<PathNodePtr> getVisitedNodes();
  std::vector<PathNodePtr> getGridPath();
