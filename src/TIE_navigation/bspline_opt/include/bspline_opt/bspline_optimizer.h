...
public:
  ...
  static const int NON_HOLONOMIC;
  ...
  void setMotionState(vector<int>& motion_state_list);
  // optional inputs
...
private:
  ...
  double kappa_max_;                    // the maximum allowable curvature
  ...
  double lambda5_;                // waypoints cost weight
  double lambda6_;                // non holonomic constraints
  ...
  vector<Eigen::Vector3d> g_nonholonomic_;
  ...
  Eigen::Vector2d calcOrthogonalComplements(Eigen::Vector2d a, Eigen::Vector2d b);
  ...
  void calcWaypointsCost(const vector<Eigen::Vector3d>& q, double& cost,
                         vector<Eigen::Vector3d>& gradient);   
  void calcNonHolomonicCost(const vector<Eigen::Vector3d>& q, double& cost,
                         vector<Eigen::Vector3d>& gradient);                      
  bool isQuadratic();
