...
private:
  enum TRAJECTORY_PLANNING_ID {
    ...
    BSPLINE_CTRL_PT = 400,
    POLY_TRAJ = 500,
    SEARCHED_GROUND_WPTS = 600,
    SEARCHED_FLYING_WPTS = 700,
    SAMPLED_GROUND_WPTS = 800,
    SAMPLED_FLYING_WPTS = 900,
    GEOASTAR = 1000,
    MOTION_PRIMITIVE = 1100
  };
  ...
  ros::Publisher geo_astar_pub_; // 5, geo Astar trajectory
...
public:
  ...
  void drawBspline(NonUniformBspline& bspline, double size, const Eigen::Vector4d& color,
                   ...
                   int id2 = 0);
  // draw hybrid waypoints
  void drawHybridSearchedWaypoints(vector<vector<Eigen::Vector3d>>& hybrid_wpts, vector<int> motion_state_list,double size,
                                        const Eigen::Vector4d& color1, const Eigen::Vector4d& color2, int id = 0);
  void drawHybridSampledWaypoints(vector<vector<Eigen::Vector3d>>& hybrid_wpts, vector<int> motion_state_list,double size,
                                        const Eigen::Vector4d& color1, const Eigen::Vector4d& color2, int id = 0);
  ...
  //motion primitives
  void drawMotionPrimitive(const vector<vector<Eigen::Vector3d>>& wpts, int best_index, 
                              double resolution, const Eigen::Vector4d& color1, const Eigen::Vector4d& color2, int id = 0);
};
}  // namespace fast_planner
#endif